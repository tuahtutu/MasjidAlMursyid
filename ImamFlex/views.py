...
def add_khutbah(request):
    ...
    if request.method == 'POST':
        ...
        if not pdf_file:
            messages.error(
                request,
                'Sila muat naik fail PDF khutbah.'
            )
        ...
        # Create JumaatPrayer record
        JumaatPrayer.objects.create(
            khutbah=khutbah,
            imam=imam
        )
...
def edit_khutbah(request, khutbah_id):
    ...
    if request.method == 'POST':
        ...
        khutbah.date = date
        khutbah.khutbah_topic = khutbah_topic
        khutbah.khatib = imam
    ...
    if jumaat:
        selected_imam = jumaat.imam.imam_id

    elif khutbah.khatib:
        selected_imam = khutbah.khatib.imam_id
...
def apply_leave(request):
    if request.method == 'POST':
        ...
        print("✅ CREATED LEAVE:", leave.request_id)
        print("=================================")
...
def ajk_info(request):
    ahli_mursyid = [
        {'name': 'HAJI AHMAD SHAFIQ BIN AHMAD MUSTAFA','photo': 'ajk/ajk1.jpeg'},
        {'name': 'MUHAMMAD ADAM BIN ABDUL HAFFIDZ','photo': 'ajk/ajk2.jpeg'},
        {'name': 'MUHAMMAD FIRDAUS BIN AZIZAN','photo': 'ajk/ajk3.jpeg'},
        {'name': 'AHMAD AKMAL BIN WAN MAHAYUDIN','photo': 'ajk/ajk4.jpeg'},
        {'name': 'ABD RAHMAN BIN MANAFF','photo': 'ajk/ajk5.jpeg'},
        {'name': 'AMIR HAKIM BIN AZMAN','photo': 'ajk/ajk6.jpeg'},
        {'name': 'MOHAMAD AL HAKEEM BIN AHMAD SOBRI','photo': 'ajk/ajk7.jpeg'},
        {'name': 'MOHD NIZAM BIN CHE ALI','photo': 'ajk/ajk8.jpeg'},
        {'name': 'KHAIRIL ANWAR BIN OTHMAN','photo': 'ajk/ajk9.jpeg'},
        {'name': 'RAZWAN BIN RAMLI','photo': 'ajk/ajk10.jpeg'},
        {'name': 'MUHAMMAD USAIZ BIN HASHIM','photo': 'ajk/ajk11.jpeg'},
        {'name': 'SALEHHUDDIN BIN HASIM','photo': 'ajk/ajk12.jpeg'},
        {'name': 'AHMAD SHUKERI BIN SENAWI','photo': 'ajk/ajk13.jpeg'},
        {'name': 'NUR \'IZZATI BINTI ABD RAHMAN','photo': 'ajk/ajk14.jpeg'},
        
    ]

    context = {
        'ahli_mursyid': ahli_mursyid,

        # ==============================
        # SEJARAH RINGKAS
        # ==============================
        'sejarah_masjid': (
            "Masjid Al-Mursyid, Guring, Pauh, Arau, Perlis pada asalnya dikenali "
            "sebagai Masjid Kampung Guring. Kariah ini telah diasaskan pada tahun "
            "1927 hasil sumbangan zakat dan fitrah melalui Majlis Agama Islam dan "
            "Adat Istiadat Melayu Perlis (MAIPs), dengan masjid asal dibina di atas "
            "tapak wakaf di Jalan Pauh-Arau.\n\n"

            "Sekitar tahun 1975 hingga 1980, masjid telah berpindah ke tapak yang "
            "digunakan sehingga kini hasil sumbangan wakaf daripada Ustaz Lebai "
            "Hassan bin Lebai Senapi. Pada awal tahun 2000, nama Masjid Kampung "
            "Guring telah ditukarkan kepada Masjid Al-Mursyid.\n\n"

            "Sejak penubuhannya, Masjid Al-Mursyid telah melalui beberapa fasa "
            "perubahan dalam kepimpinan dan pentadbiran. Pelbagai tokoh agama telah "
            "berkhidmat sebagai imam kariah bagi memastikan urusan ibadah dan "
            "kemasyarakatan penduduk setempat terus berjalan dengan baik.\n\n"

            "Pada tahun 2020, pihak MAIPs melalui Jabatan Hal Ehwal Agama Islam "
            "Perlis (JAIPs) telah melaksanakan penstrukturan semula pentadbiran "
            "Masjid Al-Mursyid. Sehingga kini, masjid terus berperanan sebagai "
            "pusat ibadah, pendidikan dan aktiviti kemasyarakatan bagi penduduk "
            "sekitar Guring, Pauh dan kawasan berdekatan."
        ),

        ## ==============================
# SEJARAH MASJID AL-MURSYID
# ==============================
'sejarah_penuh': (
    "Masjid Al-Mursyid, Guring, Pauh, Arau, Perlis pada asalnya dikenali "
    "sebagai Masjid Kampung Guring. Kariah Guring telah diasaskan pada tahun "
    "1927 dan pada peringkat awalnya terletak di tapak asal di Jalan Pauh-Arau. "
    "Pembinaan masjid tersebut dilaksanakan hasil daripada zakat dan fitrah "
    "yang dikendalikan oleh Majlis Agama Islam dan Adat Istiadat Melayu Perlis "
    "yang ketika itu dikenali sebagai Baitulmal. Pembinaan turut mendapat "
    "kebenaran untuk didirikan di atas tapak wakaf.\n\n"

    "Pada peringkat awal, antara tokoh yang diamanahkan sebagai Imam Kariah "
    "Guring ialah Tuan Haji Abdul Latiff bin Sudin, yang kemudiannya digantikan "
    "oleh anakanda beliau, Ustaz Lebai Zain bin Haji Abdul Latiff. "
    "Seterusnya, Ustaz Lebai Hassan bin Lebai Senapi menggalas tugas sebagai "
    "Imam Tua merangkap Kadi Perkahwinan, dengan bantuan Ustaz Hassan bin Jusoh "
    "sebagai Imam Muda serta Amil Zakat dan Fitrah. Tempoh perkhidmatan imam "
    "pada peringkat awal ini tidak dapat dikenal pasti dengan tepat berdasarkan "
    "rekod kajian yang ada.\n\n"

    "Sekitar tahun 1975 hingga 1980, Masjid Kampung Guring berpindah ke tapak "
    "yang digunakan sehingga hari ini. Perpindahan tersebut hasil daripada "
    "sumbangan wakaf oleh Ustaz Lebai Hassan bin Lebai Senapi. Gandingan Imam "
    "Tua dan Imam Muda kemudiannya terus berkhidmat sehingga awal tahun 2000.\n\n"

    "Pada awal tahun 2000, kepimpinan imam diteruskan oleh Ustaz Haji Ahmad "
    "bin Md Noor sebagai Imam 1. Pada tahun yang sama, nama Masjid Kampung "
    "Guring ditukarkan kepada Masjid Al-Mursyid, mengambil nama sempena "
    "anakanda kepada Ustaz Haji Ahmad bin Md Noor. Beliau kemudiannya dibantu "
    "oleh Ustaz Ramli bin Lebai Zain sebagai Imam 2, yang sebelum itu telah "
    "berkhidmat sebagai siak dan muazzin di masjid tersebut.\n\n"

    "Pada tahun 2006, Masjid Al-Mursyid menambah struktur imam kariah dengan "
    "melantik Ustaz Mohd Azuper bin Awang sebagai Imam 3. Beliau kemudiannya "
    "ditauliahkan secara tetap oleh Jabatan Kemajuan Islam Malaysia (JAKIM) "
    "pada tahun 2009.\n\n"

    "Pada tahun 2010, Ustaz Haji Ahmad bin Md Noor meninggal dunia. Jawatan "
    "Imam 1 kemudiannya dipangku oleh Ustaz Ramli bin Lebai Zain sehingga "
    "tahun 2011 sebelum beliau meletakkan jawatan atas faktor kesihatan. "
    "Pada tahun yang sama, JAIPs melantik Ustaz Md Rosdy bin Yaakob sebagai "
    "Imam 1 merangkap Penolong Pendaftar Perkahwinan (Juru Nikah), bersama "
    "Ustaz Mohd Azuper bin Awang sebagai Imam 2.\n\n"

    "Pada tahun 2019, selaras dengan ketetapan MAIPs melalui JAIPs, Ustaz Md "
    "Rosdy bin Yaakob telah ditukarkan ke Masjid Negeri Arau. Ustaz Mohd "
    "Azuper bin Awang kemudiannya mengambil alih sebagai Imam 1 merangkap "
    "Penolong Pendaftar Perkahwinan (Juru Nikah), manakala Ustaz Ahmad Shafiq "
    "bin Ahmad Mustafa Khalili dilantik sebagai Imam 2.\n\n"

    "Pada Ogos 2020, MAIPs melalui JAIPs melaksanakan penstrukturan semula "
    "pentadbiran dan pengurusan Masjid Al-Mursyid. YM Tuan Haji Syed Abu "
    "Bakar bin Syed Kamal Bharin Jamalullail dilantik sebagai pengerusi "
    "baharu, menggantikan YM Tengku Aziz bin Tengku Jaafar. Dalam perubahan "
    "struktur imam pula, Tuan Haji Kassim bin Abu, yang sebelum itu merupakan "
    "Imam di Masjid Tun Abdul Razak, Repoh, dilantik sebagai Imam 1. Namun, "
    "beliau melepaskan jawatan tersebut pada November 2020 atas faktor "
    "kesihatan.\n\n"

    "Bagi mengisi kekosongan tersebut, Ustaz Ahmad Shafiq bin Ahmad Mustafa "
    "Khalili dilantik sebagai Imam 1 merangkap Penolong Pendaftar Perkahwinan "
    "(Juru Nikah) berkuat kuasa pada Julai 2021. Ustaz Muhamad Safwan bin "
    "Bashir pula pernah berkhidmat sebagai pembantu imam pada Disember 2020 "
    "dan Januari 2021.\n\n"

    "Pada Februari 2021, Ustaz Khairul Hady bin Mohamad dilantik sebagai "
    "Imam 2. Beliau berkhidmat sehingga tahun 2022 sebelum dipindahkan ke "
    "Masjid Al-Mustaqim, Felda Rimba Emas, Padang Besar. Jawatan Imam 3 "
    "kemudiannya diberikan kepada Ustaz Muhammad Adam bin Abdul Haffidz, "
    "yang pada ketika itu turut berkhidmat sebagai Muazzin 2 Masjid "
    "Al-Mursyid. Selepas itu, beliau mengambil alih jawatan Imam 2.\n\n"

    "Bagi melengkapkan keperluan imam, pihak masjid turut melantik imam "
    "tambahan ke-3. Ustaz Muhammad Firdaus bin Mat Zainon berkhidmat selama "
    "dua bulan sebelum menyambung pengajian, manakala Ustaz Mochamad Ibnu "
    "Rusyd Al-Farabi berkhidmat selama kira-kira satu tahun sebelum kembali "
    "ke Indonesia.\n\n"

    "Pada tahun 2023, pihak Masjid Al-Mursyid melantik Ustaz Muhammad "
    "Firdaus bin Azizan sebagai Imam 3 bagi membantu Imam 1 dan Imam 2 "
    "dalam urusan pengimarahan masjid. Beliau turut memainkan peranan "
    "sebagai pentadbir masjid.\n\n"

    "Sejarah Masjid Al-Mursyid mencerminkan kesinambungan peranan para imam "
    "dan pengurusan masjid dalam membimbing masyarakat serta memastikan "
    "kelangsungan aktiviti keagamaan di Kariah Guring dari generasi ke generasi."
),

        'alamat_masjid': "Masjid Al Mursyid Guring, No. 30, RPA Pauh (Lama), 02600 Arau, Perlis",
        'no_tel_masjid': "04-9706003",
        'email_masjid': "mamgp2021@gmail.com",

    }

    return render(request, 'jemaah/ajkInfo.html', context)
